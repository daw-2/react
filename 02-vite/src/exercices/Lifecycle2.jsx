...
export default function Lifecycle2() {
  ...
  const previous = useRef(0); // Valeur conservée entre les rendus du composant

  useEffect(() => {
    if (degrees < 0 && previous.current === 0) {
      console.log('Trop froid !');
    } else if (degrees > 0 && previous.current === 0) {
      console.log('Trop chaud !');
    }

    return () => {
      previous.current = degrees; // Ancienne valeur avant update
